"""Kora V2 — Topic overlap detection for autonomous sessions.

When the user sends a message while an autonomous task is running, Kora
needs to decide whether to pause and handle the message or continue
without interruption.  ``check_topic_overlap`` computes a weighted
similarity score and maps it to an action recommendation.
...
async def check_topic_overlap(
    ...
    Args:
        ...
        autonomous_goal: High-level goal description for the entire plan.
        active_step_description: Description of the step currently executing.
        container: DI container; must expose ``embedding_service`` (optional).

    Returns:
        ``OverlapResult`` with a score and a recommended action.
    """
    try:
...
async def _embed(text: str, container: Any) -> list[float] | None:
    ...
    Returns ``None`` when the service is unavailable so callers can fall
    back to a neutral score.
    """
    service = getattr(container, "embedding_service", None)
    if service is None:
        return None
    # Support both sync and async encode() methods
    encode_fn = getattr(service, "encode_async", None) or getattr(service, "encode", None)
...
def _cosine(a: list[float] | None, b: list[float] | None) -> float:
    ...
    Using 0.5 would bias the composite score toward ambiguous/pause even
    when no semantic comparison was possible.
    """
...
def _tokenize(text: str) -> set[str]:
    """Extract content words: lowercase, alpha-only, length > 3,
